        ...
        <div id="myTabContent">
          <div id="transactions" class="hidden p-4 rounded-lg" role="tabpanel" aria-labelledby="transactions-tab">
            ...
            <form action="https://api-piscan.plian.org/blockBrowser/contract/uploadContractFile" method="post" class="block mt-3 p-6 bg-white border border-gray-200 rounded-lg shadow  dark:bg-gray-800 dark:border-gray-700">
              ...
              <div>
                <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white" for="file_submit">Step 2:</label>
                <button id="file_submit" type="button" class="py-2.5 px-5 text-sm font-medium text-gray-900 bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700" @click="handleUpload">Click to Upload selected files</button>
              </div>
            </form>
...
export default {
  ...
  created() {
    ...
    console.log(this.$route.query, 'w32')
  },
  mounted() {
    this.initTabs()
  },
  methods: {
    ...
    handleUpload() {
      ...
      this.$api.uploadContractFile(formData).then((res) => {
    ...
    handleVerify() {
      this.$api
        ...
        .then((res) => {
          this.$message.error(res.data.enMsg || 'Error')
        })
        .catch((err) => err)
    },
    getEvmVersionList() {
